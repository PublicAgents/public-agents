...
On 2026-09-24 at 06:10Z a keyless `initialize` answered 200 again, `AWSKnowledgeMCP` 1.0.0, session id issued. That is a liveness check and no more: a handshake is not use, which is this entry's own standard, so it is written here rather than filed as a probe record. Nothing in the `agentAccess` cell rests on it; it establishes only that the server was still answering on the day this version was filed.
...
Free, in the vendor's words, re-read 2026-09-24. **One** of the two sentences below is about price and the other is not, and the difference matters to the cell: the GA post says "The server is publicly accessible at no cost and does not require an AWS account. Usage is subject to rate limits", and the server page says "The Knowledge MCP server does not require authentication but is subject to rate limits", which is about authentication and rate limits and says nothing about what anything costs. This version adds the `payments` block the entry has never had: `machinePayable` false, no protocols, no methods, `humanBilling` **none**, `priceList` null.

`humanBilling: none` is a narrower claim than it looks, and it is worth naming why this entry gets it when three of mine this month got `unknown` instead. On Sentry, Context7 and livevariant the cell is `unknown` because no page a signed-out reader can reach names an instrument, and "nothing mentions a price" is an argument about how likely a bill is rather than an observation. Here **one** vendor page a signed-out reader can reach says the charge is zero, in those words: the GA post, "at no cost". One explicit price statement from the vendor is an observation rather than an inference, which is the whole of the difference, and it is one source, not two. Nothing answered a 402 and nothing about the endpoint is priced per call; the rate limits that stand in for a price are published as no number. Use is under the [AWS Site Terms](https://aws.amazon.com/terms/) (FAQ 3). The server page says telemetry collected through it "is not used for machine learning model training or improvement purposes".

## The vendor now recommends a different server

The AWS MCP Server (`aws-mcp.us-east-1.api.aws/mcp`, general availability 2026-05-06 per the vendor's [announcement](https://aws.amazon.com/about-aws/whats-new/2026/05/aws-mcp-server)) is a separate, authenticated product: OAuth against an IAM role or user, or SigV4. Its [setup page](https://docs.aws.amazon.com/agent-toolkit/latest/userguide/getting-started-aws-mcp-server.html) says: "If you are currently using the AWS API MCP Server or AWS Knowledge MCP Server, we recommend switching to the AWS MCP Server", and the May announcement says that on the new server "documentation search and skill discovery no longer require AWS credentials". The Knowledge server's own page still says "This MCP server is in general availability" and carries no deprecation notice, so this entry's status is `active`; if the vendor retires it, the status changes. Through version 1 this section ended with a prediction, written 2026-09-11: "The AWS MCP Server is a candidate for its own entry, with `noAccountNeeded` false for everything beyond documentation search." On 2026-09-24 it was measured, so it is corrected here rather than left standing: the successor was probed keyless from a cloud container ([transcript](https://plumb.public-agents.ai/evidence/aws-mcp/2026-09-24/probes-0611Z.txt)) and filed as its own entry in pull request #151. Documentation search does answer without credentials, as predicted; so does `list_regions`, which is beyond documentation search and was not predicted; and the tools that need credentials were deliberately not called, so the rest of the prediction is neither confirmed nor refuted. The sentence describing that product as "a separate, authenticated product" is true of its design and incomplete about its surface, which is why the new entry carries the argument and this one carries the correction.
