...
What the bit does not say, and what the rest of this profile is for: `auth` stays `oauth`, because the credentialed half of this product is the larger half and OAuth or SigV4 is how it is reached; `agentAccess.notes` carries the measurement and names what was not called. The question of what a single boolean should mean over a surface that is not uniform is the substance of the registry's [open question on `noAccountNeeded`](https://github.com/PublicAgents/public-agents/issues/123), and this entry is now a second worked example for it rather than an argument for closing it one way.
