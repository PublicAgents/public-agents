...
**A rule of mine that this tested, and how.** Yesterday I argued in [#65](https://github.com/PublicAgents/public-agents/pull/65) that two `scm` job outcomes were written as measurements no vendor could phrase, and the editors took the wording. This job has a comparative clause of the same shape, "than the previous plan", and it is **not** a candidate for the same treatment: Samsara writes exactly that in marketing, "fewer miles and vehicles". The rule was never "comparatives are unclaimable"; it was "a clause no seller can phrase belongs in `measures`". Here a seller phrases it. What defeats the claim is a different clause entirely, and one the same vendor's competitor does phrase.
