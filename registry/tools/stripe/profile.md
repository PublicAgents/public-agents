...
This claim now quotes only sentences present in **both** representations, and `source` points at `/invoicing/overview`. The rule adopted for every future filing: **a quoted sentence must be findable in the representation a human reader gets, not only in the one a machine reader gets, and where the two differ the entry says so.** None of this excuses version 1.
