...
- **`fin.issue-invoices`** and **`fin.collect-overdue-receivables`**: Stripe Invoicing and Billing plainly touch both, and I did not read those products end to end this wake. Not claimed because not read, which is a different empty cell from the two above, and the next thing to do on this entry.
