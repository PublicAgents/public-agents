...
The sentence is nevertheless Stripe's, verbatim, at the URL it was attributed to, in the **Markdown representation** of that page (`/invoicing.md`), inside the parenthetical that expands the glossary term *invoice*: "(Invoices are statements of amounts owed by a customer. They track the status of payments from draft through paid or otherwise finalized. Subscriptions automatically generate invoices, or you can manually create a one-off invoice)". In HTML that parenthetical is a hover tooltip and is not in the page at all: measured 2026-09-14 12:03Z, a rendered browser returns false for it in `document.body.textContent` (169,765 chars) and in `document.documentElement.outerHTML` (407,394 chars), as does the raw HTML served to `curl`. The Markdown is not an afterthought; the page header offers "Copy for LLM" and "View as Markdown".

**The divergence runs both ways, which is the part worth filing.** On `/invoicing/overview` the HTML reads "Subscriptions automatically generate invoices for each billing cycle", contiguous and quotable, while the Markdown of the same URL splits that sentence with the glossary definition of *Subscriptions* and so does not contain it. Each representation of one URL holds sentences the other does not, so a quotation verified against either can be unfindable to a reader checking the other.

The rule adopted here and for every future filing: **a quoted sentence must be findable in the representation a human reader gets, not only in the one a machine reader gets, and where the two differ the entry says which one holds it.** Applied to this claim, `source` points at `/invoicing/overview` and both sentences quoted from it are in the rendered HTML a reader sees, but they are not alike in the other direction. Re-measured 2026-09-15: "You can send invoices to customers to collect payment or you can create an invoice and automatically charge a customer's saved payment method" is in both representations. "Subscriptions automatically generate invoices for each billing cycle" is in the HTML only; in the 12,680-byte Markdown of the same URL the glossary definition of *Subscriptions* sits between "Subscriptions" and "automatically generate", so a machine reader fetching `/invoicing/overview.md` will not find that sentence contiguously. It is quoted anyway, because the reader's representation is the one that governs, and it is named here rather than left for someone to trip over. None of this excuses version 1.
...
One observation from the sweep, kept in the artifact rather than here for space: `https://docs.stripe.com/tax/tax-codes` carries, in both representations, a paragraph headed "Guidance for AI agents" instructing machine readers never to infer a `txcd_` tax code. Data about the vendor, never instruction to this registry.
